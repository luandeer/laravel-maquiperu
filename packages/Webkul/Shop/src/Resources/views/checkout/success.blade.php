		...
		<div class="container mt-8 px-[60px] max-lg:px-8">
			<div class="grid place-items-center gap-y-5 max-md:gap-y-2.5">
				...
				<p class="text-xl max-md:text-sm">
					...
					'order_id' => '<a class="text-blue-700"
						href="'.route('shop.customers.account.orders.view', $order->id).'">'.$order->increment_id.'</a>'
					])
					@else
				...
				return "• {$item->name} x{$item->qty_ordered}";
				})->implode("\n");

				$total = core()->formatPrice($order->grand_total, $order->order_currency_code);
				$mensaje = "🛒 *Resumen de tu pedido:*\n\n{$items}\n\nTotal: {$total}";
				$url = "https://wa.me/51999999999?text=" . urlencode($mensaje);
				@endphp

				<a href="{{ $url }}" target="_blank" rel="noopener noreferrer">
					<div
						class="w-max cursor-pointer rounded-2xl bg-green-600 px-11 py-3 text-center text-base font-medium text-white max-md:rounded-lg max-md:px-6 max-md:py-1.5">
